...
def filter_by_adapter(
    ...
    *,
    ...
    if not adapters or seq_column not in df.columns:
        return df
    mask: BoolS = (
        df[seq_column].astype(str).apply(lambda s: not any(adpt in s for adpt in adapters))
    )
    return df.loc[mask]
...
def remove_pcr_duplicates(
    ...
    *,
    ...
    keys: pd.Series[Any] = (
        df[[chr_col, start_col, end_col, strand_col]]
        .astype(str)
        .apply(tuple, axis=1)
    )
