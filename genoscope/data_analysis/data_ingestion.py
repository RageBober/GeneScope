...
try:
    ...
    VCF: Any | None = VCF                 # либо types.ModuleType  | None
except ImportError:
...
try:
    ...
    auto_load_gff: Callable[..., Union[pd.DataFrame, Iterator[pd.DataFrame], None]] | None = _auto_load_gff
except ImportError:
    auto_load_gff = None
...
def _return_if_not_empty(df: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
