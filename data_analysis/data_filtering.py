...
def filter_by_percentile(
    ...
    *,
    ...
    try:
        ...
        lb, ub = np.nanpercentile(values, [lower, upper])
    ...
    *,
    ...
    try:
        ...
        if not drop_outliers:
            return df
        mask: BoolS = pd.Series(dist <= dist.mean() + 2 * dist.std(), index=num_df.index)
        return df.loc[mask]
    except Exception as exc:
...
def apply_filter_pipeline(df: DF, pipeline: Pipeline) -> DF:
    ...
    for step in pipeline:
        stype = step.get("type")
        if stype not in {
            "multiple_conditions", "custom_function", "percentile", "outliers",
            "value_range", "nan_filter", "duplicates_filter",
            "correlation_filter", "variance_filter", "cluster_filter"
        }:
        ...
        try:
            if stype == "multiple_conditions":
                ...
                if not isinstance(col, str):
                    logger.warning("apply_filter_pipeline: 'value_range' missing 'column'")
                    continue
                ...
                df = filter_by_correlation(df, corr_threshold=step.get("corr_threshold", 0.95))
            elif stype == "variance_filter":
                df = filter_by_variance(df, var_threshold=step.get("var_threshold", 0.0))
