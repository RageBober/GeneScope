...
def filter_outliers(df, column, method='iqr', threshold=1.5):
    """
    Фильтрация выбросов:
      - method='iqr' (стандартный Tukey с threshold=1.5)
      - method='z-score' (threshold ~3)
      - method='mahalanobis'
      - method='isolation_forest'

    Параметры:
      column: какой столбец обрабатывать (для 'iqr' и 'z-score').
              Для 'mahalanobis' и 'isolation_forest' игнорируется
              (анализируются все числовые столбцы).
      threshold:
         - iqr: множитель (1.5, 3.0, etc.)
         - z-score: cutoff (3.0, etc.)
         - mahalanobis: cutoff (обычно ~ chi-square quantile),
         - isolation_forest: contamination=threshold (доля выбросов)
    """
    try:
        if method == 'iqr':
            ...
            # Обучаем эмпирическую ковариацию
            cov = EmpiricalCovariance().fit(num_df)
            # Вычисляем расстояния Махаланобиса
            distances = cov.mahalanobis(num_df)
            # Расстояние Махаланобиса ~ Хи-квадрат(k), где k = кол-во столбцов
            # threshold предполагаем как квантиль или число
            # Если threshold=9.21 ~ chi2(2, 0.99) (пример)
            mask = distances <= threshold
            # У нас mask соответствует num_df-индексам
            filtered_idx = num_df.index[mask]
            return df.loc[filtered_idx]

        elif method == 'isolation_forest':
            # threshold здесь трактуем как contamination (доля выбросов)
            cont = float(threshold) if 0 < threshold < 1 else 0.05
            num_df = df.select_dtypes(include=[np.number]).dropna(axis=0)
            iso = IsolationForest(contamination=cont, random_state=0)
            preds = iso.fit_predict(num_df)  # +1 normal, -1 outlier
            mask = (preds == 1)
            filtered_idx = num_df.index[mask]
            return df.loc[filtered_idx]

        else:
            print(f"Unsupported outlier method: {method}")
            return df
...
def filter_by_cluster(df, method='kmeans', n_clusters=2, drop_outliers=True):
    ...
    try:
        if method != 'kmeans':
            print(f"[filter_by_cluster] only 'kmeans' supported now.")
            return df
    ...
    for step in pipeline:
        ...
        try:
            if step_type == "multiple_conditions":
                ...
                df = filter_by_cluster(df, method=method, n_clusters=n_clust, drop_outliers=drop_out)
