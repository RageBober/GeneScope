...
def filter_by_db_snp_clinvar(df, db_col="dbSNP_ID", clinvar_col="ClinVar_ID", 
                             require_dbsnp=True, require_clinvar=False):
    """
    ...
    if require_dbsnp:
        if db_col in filtered_df.columns:
            ...
            print(f"[filter_by_db_snp_clinvar] Колонка '{db_col}' не найдена, пропускаем DB SNP фильтр.")

    if require_clinvar:
        if clinvar_col in filtered_df.columns:
            filtered_df = filtered_df[filtered_df[clinvar_col].notna()]
        else:
            print(f"[filter_by_db_snp_clinvar] Колонка '{clinvar_col}' не найдена, пропускаем ClinVar фильтр.")
