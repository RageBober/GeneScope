...
def fill_missing_with_ml(df: pd.DataFrame, columns: list) -> pd.DataFrame:
    ...
    for col in columns:
        ...
        if known_data[col].dropna().empty:
            logger.warning(f"[fill_missing_with_ml] Столбец '{col}' полностью пуст, пропуски не заполним.")
            continue
        ...
        if pd.api.types.is_numeric_dtype(df_copy[col]):
            ...
            if known_data[col].nunique() <= 1:
                logger.warning(f"[fill_missing_with_ml] '{col}' имеет <=1 класс, LogisticRegression не обучим.")
                continue
        ...
        if sub_known.empty:
            logger.warning(f"[fill_missing_with_ml] Нет строк без пропусков в features для '{col}'.")
            continue
        ...
        try:
            ...
            logger.error(f"[fill_missing_with_ml] Ошибка обучения модели для '{col}': {fit_err}",
                        exc_info=True)
            continue
        ...
        if sub_missing.empty:
            logger.warning(f"[fill_missing_with_ml] Нет строк без NaN в features для предикта '{col}'.")
            continue
        ...
        try:
            ...
            logger.error(f"[fill_missing_with_ml] Модель для '{col}' не обучена (NotFittedError).",
                        exc_info=True)
            continue
        except Exception as pred_err:
            logger.error(f"[fill_missing_with_ml] Ошибка предсказания для '{col}': {pred_err}",
                        exc_info=True)
            continue

        df_copy.loc[X_test.index, col] = preds
        logger.info(f"[fill_missing_with_ml] Пропуски в '{col}' заполнены моделью {type(model).__name__}.")
