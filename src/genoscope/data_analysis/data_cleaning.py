...
# ─── LOCAL / INTERNAL ───────────────────────────────────────────────
# from mlmodel.data_cleaning_AI.ml_imputation import fill_missing_with_ml  # TODO: Fix import path
# Временно используем локальную реализацию
    ...
    *,
    ...
    try:
        if method in ["ffill", "bfill"]:
            ...
            # df_copy = fill_missing_with_ml(df_copy, columns)  # TODO: Fix import
            # Временно используем локальную реализацию
            df_copy = _fill_missing_with_ml(df_copy, columns)
...
# --------------------------------------------------------------------------------------
def _numeric_cols(
    df: pd.DataFrame, cols: list[str], allow_mixed: bool, ctx: str
) -> list[str]:
    ok: list[str] = []
    for c in cols:
