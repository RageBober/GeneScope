...
# ──────────────────────────────────────────────────────────────
# 1. Быстрая валидация GFF
# ──────────────────────────────────────────────────────────────
def validate_gff_file(file_path: str, lines_to_check: int = 10) -> tuple[bool, str]:
    """
...
def load_gff_advanced(
    ...
    *,
    ...
    try:
        db = gffutils.create_db(file_path, dbfn=dbfn, force=force, keep_order=True)
    except Exception as e:
